...
class Seller extends Account{
  ...
  Seller({String name, String email, String telepon, String registrationDate, String uid, int role, this.location, this.namaPerusahaan})
  ...
  Stream<List<Item>> listProduct(){
    ...
    .map((event) => event.documents.map((doc) => Item.fromDb(doc.data)).toList());
  }
}
...
class PejabatPengadaan extends Account{
  ...
  PejabatPengadaan({String name, String email, String telepon, String registrationDate, String uid, int role, this.unit})
  :super(name: name, email: email, telepon: telepon, registrationDate: registrationDate, uid: uid, role: role);
